...
namespace DTcms.DAL
{
    ...
    public partial class member_albums
    {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.member_albums> GetList(int member_id, int Top)
        {
            List<Model.member_albums> modelList = new List<Model.member_albums>();
            ...
            strSql.Append(" FROM member_albums ");
            ...
            if (rowsCount > 0)
            {
                Model.member_albums model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new Model.member_albums();
        ...
        /// </summary>
        public void DeleteList(SqlConnection conn, SqlTransaction trans, List<Model.member_albums> models, int member_id)
        {
            StringBuilder idList = new StringBuilder();
            if (models != null)
            {
                foreach (Model.member_albums modelt in models)
                {
                    if (modelt.id > 0)
                    {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,thumb_path,original_path from member_albums where member_id=" + member_id);
            ...
            {
                int rows = DbHelperSQL.ExecuteSql(conn, trans, "delete from member_albums where id=" + dr["id"].ToString()); //删除数据库
                if (rows > 0)
                {
        ...
        /// 删除相册图片
        /// </summary>
        public void DeleteFile(List<Model.member_albums> models)
        {
            if (models != null)
            {
                foreach (Model.member_albums modelt in models)
