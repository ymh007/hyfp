...
namespace DTcms.Web.tools
{
    ...
    public class admin_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (action)
            {
                ...
                case "manager_validate": //验证管理员用户名是否重复
                    ...
                    break;
                case "xiehui_validate": //验证协会账号是否重复
                    xiehui_validate(context);
                    break;
                case "member_validate": //验证协会会员是否重复
                    member_validate(context);
                    break;
        ...
        #region 验证协会账号是否重复========================
        private void xiehui_validate(HttpContext context)
        {
            string no = DTRequest.GetString("param");
            if (string.IsNullOrEmpty(no))
            {
                context.Response.Write("{ \"info\":\"请输入协会账号\", \"status\":\"n\" }");
                return;
            }
            BLL.xiehui bll = new BLL.xiehui();
            if (bll.Exists(no))
            {
                context.Response.Write("{ \"info\":\"协会账号已被占用，请更换！\", \"status\":\"n\" }");
                return;
            }
            context.Response.Write("{ \"info\":\"协会账号可使用\", \"status\":\"y\" }");
            return;
        }
        #endregion

        #region 验证协会会员是否重复========================
        private void member_validate(HttpContext context)
        {
            string no = DTRequest.GetString("param");
            if (string.IsNullOrEmpty(no))
            {
                context.Response.Write("{ \"info\":\"请输入协会会员证号码\", \"status\":\"n\" }");
                return;
            }
            BLL.member bll = new BLL.member();
            if (bll.Exists(no))
            {
                context.Response.Write("{ \"info\":\"协会会员证号码已被占用，请更换！\", \"status\":\"n\" }");
                return;
            }
            context.Response.Write("{ \"info\":\"协会会员证号码可使用\", \"status\":\"y\" }");
            return;
        }
        #endregion
        ...
        {
            Model.xiehui adminModel = new ManagePage().GetAdminInfo(); //获得当前登录管理员信息
            if (adminModel == null)
            {
                return;
            }
        ...
        {
            //取得管理员登录信息
            Model.xiehui adminInfo = new Web.UI.ManagePage().GetAdminInfo();
            if (adminInfo == null)
            {
            ...
            switch (edit_type.ToLower())
            {
                case "order_confirm": //确认订单
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.no, DTEnums.ActionEnum.Confirm.ToString(), "确认订单号:" + model.order_no); //记录日志
                    #region 发送短信或邮件============================
                ...
                case "order_payment": //确认付款
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.no, DTEnums.ActionEnum.Confirm.ToString(), "确认付款订单号:" + model.order_no); //记录日志
                    #region 发送短信或邮件
                    if (orderConfig.confirmmsg > 0)
                    {
                        switch (orderConfig.confirmmsg)
                ...
                case "order_express": //确认发货
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.no, DTEnums.ActionEnum.Confirm.ToString(), "确认发货订单号:" + model.order_no); //记录日志
                    #region 发送短信或邮件============================
                ...
                case "order_complete": //完成订单=========================================
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.no, DTEnums.ActionEnum.Confirm.ToString(), "确认交易完成订单号:" + model.order_no); //记录日志
                    #region 发送短信或邮件=========================
                ...
                case "order_cancel": //取消订单==========================================
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.no, DTEnums.ActionEnum.Cancel.ToString(), "取消订单号:" + model.order_no); //记录日志
                ...
                case "order_invalid": //作废订单==========================================
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.no, DTEnums.ActionEnum.Invalid.ToString(), "作废订单号:" + model.order_no); //记录日志
                ...
                case "edit_accept_info": //修改收货信息====================================
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.no, DTEnums.ActionEnum.Edit.ToString(), "修改收货信息，订单号:" + model.order_no); //记录日志
                ...
                case "edit_order_remark": //修改订单备注=================================
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.no, DTEnums.ActionEnum.Edit.ToString(), "修改订单备注，订单号:" + model.order_no); //记录日志
                ...
                case "edit_real_amount": //修改商品总金额================================
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.no, DTEnums.ActionEnum.Edit.ToString(), "修改商品金额，订单号:" + model.order_no); //记录日志
                ...
                case "edit_express_fee": //修改配送费用==================================
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.no, DTEnums.ActionEnum.Edit.ToString(), "修改配送费用，订单号:" + model.order_no); //记录日志
                ...
                case "edit_payment_fee": //修改支付手续费=================================
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.no, DTEnums.ActionEnum.Edit.ToString(), "修改支付手续费，订单号:" + model.order_no); //记录日志
                ...
                case "edit_invoice_taxes": //修改发票税金=================================
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.no, DTEnums.ActionEnum.Edit.ToString(), "修改订单发票税金，订单号:" + model.order_no); //记录日志
