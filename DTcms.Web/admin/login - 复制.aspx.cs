...
namespace DTcms.Web.admin
{
    public partial class login : System.Web.UI.Page
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            Model.xiehui model = bll.GetModel(userName, userPwd, true);
            if (model == null)
            {
            ...
            {
                new BLL.manager_log().Add(model.id, model.no, DTEnums.ActionEnum.Login.ToString(), "用户登录");
            }
            //写入Cookies
            Utils.WriteCookie("DTRememberName", model.no, 14400);
            Utils.WriteCookie("AdminName", "DTcms", model.no);
