...
namespace DTcms.DAL
{
    ...
    /// 图片相册
    /// </summary>
    public partial class daikuan_albums
    {

        /// <summary>
        /// 获得数据列表
        /// </summary>
        public List<Model.daikuan_albums> GetList(int daikuan_id, int Top)
        {
            List<Model.daikuan_albums> modelList = new List<Model.daikuan_albums>();
            ...
            strSql.Append(" FROM daikuan_albums ");
            ...
            if (rowsCount > 0)
            {
                Model.daikuan_albums model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new Model.daikuan_albums();
        ...
        /// </summary>
        public void DeleteList(SqlConnection conn, SqlTransaction trans, List<Model.daikuan_albums> models, int daikuan_id)
        {
            StringBuilder idList = new StringBuilder();
            if (models != null)
            {
                foreach (Model.daikuan_albums modelt in models)
                {
                    if (modelt.id > 0)
                    {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,thumb_path,original_path from daikuan_albums where daikuan_id=" + daikuan_id);
            ...
            {
                int rows = DbHelperSQL.ExecuteSql(conn, trans, "delete from daikuan_albums where id=" + dr["id"].ToString()); //删除数据库
                if (rows > 0)
                {
        ...
        /// 删除相册图片
        /// </summary>
        public void DeleteFile(List<Model.daikuan_albums> models)
        {
            if (models != null)
            {
                foreach (Model.daikuan_albums modelt in models)
