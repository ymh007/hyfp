...
namespace DTcms.DAL
{
    ...
    /// 图片相册
    /// </summary>
    public partial class member_family
    {

        /// <summary>
        /// 获得数据列表
        /// </summary>
        public List<Model.member_family> GetList(int account_id, int Top)
        {
            List<Model.member_family> modelList = new List<Model.member_family>();
            ...
            strSql.Append(" FROM member_family ");
            ...
            if (rowsCount > 0)
            {
                Model.member_family model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new Model.member_family();
        ...
        /// </summary>
        public void DeleteList(SqlConnection conn, SqlTransaction trans, List<Model.member_family> models, int account_id)
        {
            StringBuilder idList = new StringBuilder();
            if (models != null)
            {
                foreach (Model.member_family modelt in models)
                {
                    if (modelt.id > 0)
                    {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,member_id,name,gender,relationship,birthday,education  from member_family where member_id=" + account_id);
