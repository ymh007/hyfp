...
namespace DTcms.DAL
{
    ...
    public partial class article_category
    {
            ...
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        strSql.Append("channel_id,title,call_index,parent_id,class_list,class_layer,sort_id,link_url,img_url,content,seo_title,seo_keywords,seo_description,page_type)");
                        ...
                        SqlParameter[] parameters = {
                                ...
                                new SqlParameter("@page_type",SqlDbType.Int,4)};
                        ...
                        parameters[13].Value = model.page_type;
            ...
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        strSql.Append("page_type=@page_type");
                        strSql.Append(" where id=@id");
                        SqlParameter[] parameters = {
					            new SqlParameter("@channel_id", SqlDbType.Int,4),
					            new SqlParameter("@title", SqlDbType.NVarChar,100),
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,channel_id,title,call_index,parent_id,class_list,class_layer,sort_id,link_url,img_url,content,seo_title,seo_keywords,seo_description,page_type");
        ...
        public DataTable GetChildList(int parent_id, int channel_id, int? top)
        {
            ...
            strSql.Append(" id,channel_id,title,call_index,parent_id,class_list,class_layer,sort_id,link_url,img_url,content,seo_title,seo_keywords,seo_description,page_type");
            strSql.Append(" from " + databaseprefix + "article_category");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,channel_id,title,call_index,parent_id,class_list,class_layer,sort_id,link_url,img_url,content,seo_title,seo_keywords,seo_description,page_type");
            strSql.Append(" from " + databaseprefix + "article_category");
        ...
        public Model.article_category DataRowToModel(DataRow row)
        {
            ...
            if (row != null)
            {
                ...
                if (row["page_type"] != null && row["page_type"].ToString() != "")
                {
                    model.page_type = int.Parse(row["page_type"].ToString());
                }
            }
            return model;
        }
        ...
        private void GetChilds(DataTable oldData, DataTable newData, int parent_id, int channel_id)
        {
            ...
            for (int i = 0; i < dr.Length; i++)
            {
                ...
                row["page_type"] = int.Parse(dr[i]["page_type"].ToString());
