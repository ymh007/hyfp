...
namespace DTcms.Web.UI
{
    public class UpLoad
    {
        ...
        /// <param name="dicInfo">临时文件夹的文件列表</param>
        /// <returns></returns>
        public string MergeFiles(string fileName, int filesize, DirectoryInfo dicInfo)
        {
            try
            {
                string fileExt = Utils.GetFileExt(fileName); //文件扩展名，不含“.”
                int fileSize = filesize; //获得文件大小，以字节为单位
                ...
                SaveMergeFile(dicInfo, fullUpLoadPath, newFileName);
                return "{\"chunked\" : true, \"status\": 1, \"msg\": \"上传文件成功！\", \"name\": \""
        ...
        /// 合并分片文件
        /// </summary>
        /// <param name="dicInfo">分片文件目录</param>
        /// <param name="fullUpLoadPath">上传目录的物理路径</param>
        /// <param name="newFilePath">文件名</param>
        private static void SaveMergeFile(DirectoryInfo dicInfo, string fullUpLoadPath, string newFileName)
        {
            FileInfo[] files = dicInfo.GetFiles();
            foreach (FileInfo file in files.OrderBy(f => int.Parse(f.Name)))
            {
                FileStream addFile = new FileStream(fullUpLoadPath + newFileName, FileMode.Append, FileAccess.Write);
