...
namespace DTcms.Web.admin.article
{
    public partial class article_edit : Web.UI.ManagePage
    {
        ...
        private void ShowInfo(int _id)
        {
            ...
            BLL.xiehui managerBll = new BLL.xiehui();
            string realName = managerBll.GetRealName(model.user_name);
            txtCreatName.Text = realName;
        ...
        private bool DoAdd()
        {
            ...
            model.user_name = GetAdminInfo().no; //获得当前登录用户名
            ...
            if (albumArr != null && albumArr.Length > 0)
            {
                ...
                for (int i = 0; i < albumArr.Length; i++)
                {
                    ...
                    if (imgArr.Length == 3)
                    {
                        ...
                        {
                            ls.Add(new Model.article_albums { original_path = imgArr[1], thumb_path = imgArr[2], link_url = link_urlArr[i] });
                        }
            ...
            if (attachFileNameArr != null && attachFilePathArr != null && attachFileSizeArr != null && attachFileNameArr.Length > 0
                && attachFilePathArr.Length > 0 && attachFileSizeArr.Length > 0)
            {
            ...
            if (albumArr != null)
            {
                ...
                for (int i = 0; i < albumArr.Length; i++)
                {
                    ...
                    if (imgArr.Length == 3)
                    {
                        ...
                        {
                            ls.Add(new Model.article_albums { id = img_id, article_id = _id, original_path = imgArr[1], thumb_path = imgArr[2], remark = remarkArr[i], link_url = linkUrlArr[i] });
                        }
                        ...
                        {
                            ls.Add(new Model.article_albums { id = img_id, article_id = _id, original_path = imgArr[1], thumb_path = imgArr[2], link_url = linkUrlArr[i] });
                        }
                        else
                        {
