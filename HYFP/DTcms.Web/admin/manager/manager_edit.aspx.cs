...
namespace DTcms.Web.admin.manager
{
    public partial class manager_edit : Web.UI.ManagePage
    {
        ...
        private void CreateOtherField()
        {
            ...
            htmlDT.InnerHtml = "管理类别";

            var categoryList = new BLL.article_category().GetAllList(0);

            CheckBoxList cblControl = new CheckBoxList();
            cblControl.ID = "field_control_category";
            ...
            {
                var listItem = new ListItem(item["title"].ToString(), item["id"].ToString());
                if (item["parent_id"].ToString() == "0")
                {
                    listItem.Attributes.Add("style", "style='font-weight:bold'");
                }
                //赋值选项
                cblControl.Items.Add(listItem);
            }
            //创建一个Label控件
