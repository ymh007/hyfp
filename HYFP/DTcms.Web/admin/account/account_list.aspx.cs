...
namespace DTcms.Web.admin.account
{
    public partial class account_list : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("account_manager", DTEnums.ActionEnum.View.ToString()); //检查权限
                Model.xiehui model = GetAdminInfo(); //取得当前管理员信息
                RptBind("id>0", "date asc,id desc");
            }
        }
        ...
        {
            ChkAdminLevel("account_list", DTEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.account bll = new BLL.account();
