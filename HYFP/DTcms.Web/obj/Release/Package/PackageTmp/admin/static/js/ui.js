...
/*添加收藏
<a title="收藏本站" href="javascript:;" onClick="addFavoritepage('ui前端框架','');">收藏本站</a>
*/
/*收藏主站*/
...
/*模拟下拉菜单*/
jQuery.select = function(divselectid,inputselectid) {
...
/*返回顶部*/
var $backToTopEle=$('<a href="javascript:void(0)" class="iconfont toTop" title="返回顶部" alt="返回顶部" style="display:none">&#xe684;</a>').appendTo($("body")).click(function(){
...
$(function(){
	...
	$(document).on("keydown",".tags-input",function(event){
		$(this).next().hide();
		...
		if(v!=''){
			if(event.keyCode==13||event.keyCode==108||event.keyCode==32){
				$('<span class="tags-token">'+v+'</span>').insertBefore($(this).parents(".tags").find(".tags-iptwrap"));
				$(this).val("");
				gettagval(this);
			}
		}else{
			if(event.keyCode==8){
				if(token.length>=1){
					...
					$(this).parents(".tags").find(".tags-lable").show();
					return false;
				}
	...
	$(document).on("click",".tags-has span",function(){
		var taghasV = $(this).text();
		taghasV=taghasV.replace(/(^\s*)|(\s*$)/g,"");
		$('<span class="tags-token">'+taghasV+'</span>').insertBefore($(this).parents(".tags").find(".tags-iptwrap"));
		gettagval(this);
		$(this).parents(".tags").find(".tags-input").focus();
	});
	$(document).on("click",".tags-token",function(){
		...
		$(this).remove();
		switch(token.length){
			case 1 : it.find(".tags-lable").show();
			break;
		}
		var str ="";
