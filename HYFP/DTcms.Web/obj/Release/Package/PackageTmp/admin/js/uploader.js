    ...
    $.fn.InitUploader = function (b) {
        ...
        var fun = function (parentObj) {
            ...
            uploader.on('uploadSuccess', function (file, data) {
                if (data.chunked) {//分片上传
                    parentObj.siblings(".upload-name").val("分片合并...");
                    parentObj.siblings(".upload-path").val("分片合并...");
                    $.post("/tools/upload_ajax.ashx?action=MergeFiles", { guid: GUID, filename: file.name, filepath: file.path, filesize: file.size },
                                  function (data) {
                                      data = $.parseJSON(data);
                                      if (data.status == '0') {
                                          var progressObj = parentObj.children(".upload-progress");
                    ...
                    if (data.status == '1') {
                        ...
                        if (!p.multiple) {
                            ...
                            addImage(parentObj, data.path, data.thumb, p.index);
                        }
                        var progressObj = parentObj.children(".upload-progress");
                        progressObj.children(".txt").html("上传成功：" + file.name);
                    }
...
function addImage(targetObj, originalSrc, thumbSrc,index) {
    ...
    if (index==1) {
        var newLi = $('<li>'
          + '<input type="hidden" name="hid_photo_cw_name" value="0|' + originalSrc + '|' + thumbSrc + '" />'
          + '<input type="hidden" name="hid_photo_cw_remark" value="" />'
          + '<input type="hidden" name="hid_photo_cw_link_url" value="" />'
        ...
        var newLi = $('<li>'
          ...
          + '<input type="hidden" name="hid_photo_link_url" value="" />'
          + '<div class="img-box" onclick="setFocusImg(this);">'
          + '<img src="' + thumbSrc + '" bigsrc="' + originalSrc + '" />'
          + '<span class="remark"><i>暂无描述...</i></span>'
          + '</div>'
...
//设置图片链接地址
function setLinkurl(obj) {
    var parentObj = $(obj); //父对象
    var hidlinkUrlObj = parentObj.prevAll("input[name='hid_photo_link_url']").eq(0); //取得隐藏值
    var d = parent.dialog({
        title: "链接地址",
        content: '<textarea id="ImagelinkUrl" style="margin:10px 0;font-size:12px;padding:3px;color:#000;border:1px #d2d2d2 solid;vertical-align:middle;width:300px;height:50px;">' + hidlinkUrlObj.val() + '</textarea>',
        button: [{
            value: '链接地址',
            callback: function () {
                var linkUrlObj = $('#ImagelinkUrl', parent.document);
                //if (linkUrlObj.val() == "") {
                //    parent.dialog({
                //        title: '提示',
                //        content: '亲，总该写点什么吧？',
                ...
                //    return false;
                //}
                hidlinkUrlObj.val(linkUrlObj.val());
                parentObj.siblings(".img-box").children(".linkUrl").children("i").html(linkUrlObj.val());
            },
            autofocus: true
        }]
    }).showModal();
