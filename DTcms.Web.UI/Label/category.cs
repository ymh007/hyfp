...
namespace DTcms.Web.UI
{
    public partial class BasePage : System.Web.UI.Page
    {
        ...
        /// <returns></returns>
        protected int get_categoryId_id(int parent_id, string channel_name)
        {
            var dt = new BLL.article_category().GetList(parent_id, channel_name);
            if (dt != null)
            {
                if (dt.Rows.Count > 0)
        ...
        /// <param name="channel_name">频道名称</param>
        /// <param name="parent_id">父类别ID</param>
        /// <returns>Model.channel</returns>
        protected Model.channel get_channel_model(int channel_id)
        {
            return new BLL.channel().GetModel(channel_id);
        }
