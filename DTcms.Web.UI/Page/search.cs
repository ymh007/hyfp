...
namespace DTcms.Web.UI.Page
{
    public partial class search : Web.UI.BasePage
    {
        ...
        protected DataTable get_search_list(int _pagesize, out int _totalcount)
        {
            ...
            DataSet ds = new BLL.article().GetSearch(channel, _pagesize, page, "(content like '%" + keyword + "%' or zhaiyao like '%" + keyword + "%')", "add_time desc,id desc", out _totalcount);
            if (ds.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    dr["user_name"] = dr1["user_name"]; //用户名
                    dt.Rows.Add(dr);
                    //}
                }
            }
